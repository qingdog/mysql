...
> case when表达式1 then语句1 [when2 then2 ...] else语句
>
> case value值 when值1 then语句1 [when2 then2 ...] else语句
...
begin
	...
	sum:loop
		...
		if n%2 = 1 then
			set n := n - 1;
			iterate sum;-- 进入指定循环迭代（不再往后执行，开始下一轮（类似continue））
		end if;
...
我们可以通过下面的SQL，来**查看performance（性能）数据库中的元数据锁metadata_locks**的情况：
...
客户端一，在执行DML操作时，会对涉及的行***加行锁，同时也会对该表加上意向锁***。（意向锁属于表级）
...
可以**通过数据锁表data_locks，查看意向锁及行锁**的加锁情况：
...
表空间是InnoDB存储引擎逻辑结构的最高层， 如果用户启用了参数 **innodb_file_per_table**（在8.0版本中默认开启） ，则**每张表都会有一个表空间（xxx.ibd）**，一个mysql实例可以对应多个表空间，用于存储记录、索引等数据。
...
* Roll_pointer（回滚指针）：每次对某条引记录进行改动时，都会把旧的版本写入到undo日志中，然后这个隐藏列就相当于一个指针，可以通过它来找到该记录修改前的信息。
...
在专用服务器上，**通常将多达80％的物理内存分配给缓冲池** 。参数大小：

```sql
MySQL (none)@localhost:(none)> show variables like 'innodb_buffer_pool_size'
+-------------------------+-----------+
| Variable_name           | Value     |
+-------------------------+-----------+
...
与聚集索引不同，二级索引通常是非唯一的，并且以相对随机的顺序插入二级索引。同样，删除和更新 可能会影响索引树中不相邻的二级索引页，如果每一次都操作磁盘，会造成大量的磁盘IO。有了ChangeBuffer之后，我们可以在**缓冲池中进行合并处理，减少磁盘IO**。
...
```sql
MySQL (none)@localhost:(none)> show variables like 'innodb_adaptive_hash_index%'
+----------------------------------+-------+
| Variable_name                    | Value |
+----------------------------------+-------+
...
```sql
MySQL (none)@localhost:(none)> select @@innodb_log_buffer_size,@@innodb_flush_log_at_trx_commit
+--------------------------+----------------------------------+
...
```sql
MySQL (none)@localhost:(none)> show variables like 'innodb_data_file_path'
+-----------------------+------------------------+
| Variable_name         | Value                  |
+-----------------------+------------------------+
...
```sql
MySQL (none)@localhost:(none)> show variables like 'innodb_file_per_table'
+-----------------------+-------+
| Variable_name         | Value |
+-----------------------+-------+
...
```sql
CREATE TABLESPACE 表空间名 ADD DATAFILE '文件名' ENGINE = innodb;
```
![](./media/image178.jpeg)
...
```sql
CREATE TABLE a(id int)engine=innodb TABLESPACE 表空间名;
```
...
核心后台线程，**负责调度其他线程**，还负责**将缓冲池中的数据异步刷新到磁盘中**，保持数据的一致性， 还包括脏页的刷新、合并插入缓存、undo页的回收 。
...
主要用于回收事务已经提交了的undo log，在**事务提交之后，undo log可能不用了，就用它来回收**。
...
而对于这四大特性，实际上分为两个部分。 其中的**原子性、一致性、持久化，实际上是由InnoDB中的两份日志来保证**的，一份是redo log日志，一份是undo log日志。 而**隔离性是通过数据库的锁， 加上MVCC多版本并发控制来保证的**。
...
因为在业务操作中，我们操作数据一般都是随机读写磁盘的，而不是顺序读写磁盘。 而**redo log在往磁盘文件中写入数据，由于是日志文件，所以都是顺序写的。顺序写的效率，要远大于随机写。** 这种先写日志的方式，称之为 WAL（Write-Ahead Logging）。

> 默认情况下，每个Redo Log文件的大小是 50MB。通过修改 `innodb_log_files_in_group` 参数来指定数量。通过修改 `innodb_log_file_size` 参数来指定大小。这些参数的值应该根据具体的应用需求和系统配置进行调整。
>
> 并行写入操作需要至少三个或更多的Redo Log日志文件才能生效。这是因为并行写入需要将事务的Redo Log记录分散写入到多个日志文件中，从而实现并行写入操作。
>
> **"刷新到磁盘"和"落盘"**是两个概念，它们在数据库中的意义略有不同。
>
> 1. 刷新到磁盘（Flush to Disk）：在数据库中，刷新到磁盘是指将数据从内存缓冲区（如Buffer Pool）写入到磁盘中对应的数据文件的过程。当数据在内存中被修改时，为了保持数据的持久性和一致性，需要将这些被修改的数据写入到磁盘上的数据文件中。刷新到磁盘可以通过操作系统的写入操作实现，将内存中的数据写入到对应的磁盘块中，以便在断电或系统崩溃等异常情况下恢复数据。
>
> 2. 落盘（Durability）：在数据库中，落盘是指将数据持久化写入到磁盘的过程，确保数据在异常情况下不会丢失。当一个事务提交后，涉及到的数据必须被持久化存储到磁盘上的数据文件中，以保证数据的持久性和一致性。落盘包括两个关键步骤：首先，将事务的Redo Log日志记录刷新到磁盘，以确保日志记录的持久性；然后，将事务所修改的数据页刷新到磁盘，确保数据的持久性。
>
> 在这两个概念中，"刷新到磁盘"更侧重于将内存中的数据写入到磁盘的操作，它可以在事务执行过程中发生，以确保数据的一致性和持久性。而"落盘"则更广义地指代将事务的Redo Log日志记录和相关数据持久化写入到磁盘，以确保数据在异常情况下的持久性。
>
> 需要注意的是，"刷新到磁盘"是实现"落盘"过程的一部分，它确保了数据在内存和磁盘之间的同步。而"落盘"则更全面地描述了将事务的Redo Log日志记录和数据写入到磁盘的过程。
...
```sh
cd /var/lib/mysql/itcast #数据库名

ibd2sdi stu.ibd #stu表名（表空间）
```

查看到的表结构信息中，有一栏 columns，在其中我们会看到处理我们建表时指定的字段以外，还有额外的两个字段分别是：DB_TRX_ID 、 DB_ROLL_PTR ，因为该表有主键，所以没有DB_ROW_ID 隐藏字段。
...
> 如果当前事务id小于readview读视图的最大id，说明该事务已提交，可读取视图。若不满足则继续匹配undo log回滚日志版本链的下一条。
...
* 先匹配![](./media/image210.jpeg)这条记录，这条记录对应的 trx_id 为4，也就是将4带入右侧的匹配规则中。 ①不满足 ②不满足 ③不满足 ④也不满足 ， 都不满足，则继续匹配undo log版本链的下一条。
...
我们看到，**在RR隔离级别下，只是在事务中第一次快照读时生成ReadView**，后续都是复用该ReadView，那么既然ReadView都一样， ReadView的版本链匹配规则也一样， 那么最终**快照读返回的结果也是一样的**。
...
* db_trx_id：每次对某条记录进行改动时，都会把对应的事务id赋值给trx_id隐藏列。（事务id）
* db_roll_pointer：每次对某条引记录进行改动时，都会把旧的版本写入到undo日志中，然后这个隐藏列就相当于一个指针，可以通过它来找到该记录修改前的信息。（回滚指针）
* db_row_id（虚拟行id）
...
|information_schema|提供了访问数据库**元数据**的各种表和视图，包含数据库、表、字段类型及访问权限等|
|performance_schema|为MySQL服务器运行时状态提供了一个底层监控功能，主要用于收集数据库服务器**性能**参数|
|sys|包含了一系列方便 DBA 和开发人员利用 performance_schema性能数据库进行**性能调优和诊断的视图**|
